...
@section('content')
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>{{ __('Approved Permanent Items Page') }}</h1>
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active text-capitalize">{{ __('Approved Permanent Items') }}</li>
                </ol>
            </nav>
        </div>
                                    ...
                                    @foreach ($test as $test)
                                        <tr>
                                            ...
                                            <td>
                                                    ...
                                                    <ul class="dropdown-menu text-center ">
                                                        <li>
                                                            {{-- @if ($test->out_of_store == 'no') --}}
                                                                <a href="{{ route('give_per_item',['req_id' => $test->r_id, 'item_id'=> $test->r_item_id] ) }}"
                                                ...
                                                @elseif ($test->out_of_store == 'Received')
                                                    ...
                                                    <ul class="dropdown-menu text-center ">
                                                        <li>
                                                            {{-- @if ($test->out_of_store == 'no') --}}
                                                                <a href="{{ route('return_per_item',['req_id' => $test->r_id, 'item_id'=> $test->r_item_id] ) }}"
                                                                    class="text-dark  tooltip-test"
                                                                    title="Allow user to the system">{{__('Returned')}}</a>
