...
@section('content')
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>{{ __('Employee Education') }}</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item text-capitalize"><a href="{{ route('home') }}">{{ __('Home') }}</a></li>
                    <li class="breadcrumb-item text-capitalize"><a href="{{ route('home') }}">{{ __('HR Management') }}</a></li>
                    <li class="breadcrumb-item active text-capitalize">{{ __('Employee Education') }}</li>
                </ol>
            </nav>
        </div>
                                ...
                                @can('Personal_information access')
                                    <thead class="text-capitalize">
                                        <tr>
                                            ...
                                            <th scope="col">{{__('Elementary School')}}</th>
                                            <th scope="col">{{ __('High School') }}</th>
                                            <th scope="col">{{__('Preparatory School')}}</th>
                                            <th scope="col">{{__('Higher Commission')}}</th>
                                            <th scope="col">{{__('Education Level')}}</th>
                                    ...
                                    <tbody>
                                        @foreach ($test as $test)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $test->elementary_school }}</td>
                                                <td>{{ $test->high_school }}</td>
                                                <td>{{ $test->prep_school }}</td>
                                                <td>{{ $test->higher_commission }}</td>
                                                <td>{{ $test->education_level }}</td>
